...
#cyclic pattern: long period pattern, usually longer than seasonal pattern
```

Is a linear regression an appropriate model for the data in this case? Why or why not?

> No, limits seasonality
...
```{r}
# ts for time series function
# there should be no NA data, but cannot just drop na data cause it will influence missing message,
#frequency: weekly data: f = 7; yearly data: f = 365
...
# remainder(error) + trend + seasonal = data
...
ggplot(ClearCreek_Components) +
  ...
  geom_hline(yintercept = 0, lty = 2) +
  ylab(expression("Discharge (ft"^3*"/s)"))
# it could have negative seasonal numbers
```
