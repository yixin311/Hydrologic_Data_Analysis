...
watersheds, depth and flow, snow mapping
...
* **y**: latitude (required)
* **z**: altitude (optional) contour lines, color
...
* **Point** <add notes here> sampling locations, have lat and lon
* **Line** <add notes here> at least 2 point to make a line, connectivity between different things
* **Polygon** <add notes here> at least 3 points, birng back to the original point, have certain area, pixels classification, larger areas
...
LAGOScombined <- 
  left_join(LAGOSnutrient, LAGOSlocus) %>% # LAGOSnutrient does not have locations, if you don't specify the common colomns, R will choose the colomn with the same name
  left_join(., LAGOSlimno) %>%
  left_join(., LAGOSstate) %>%
  filter(!is.na(state)) %>% #fills out locations where there are no state
...
```{r}
secchi.spatial <- st_as_sf(secchi.summary, coords = c("long", "lat"), crs = 4326) # st_as_sf

Secchiplot <- ggplot() +
  geom_sf(data = states.subset, fill = "white") +
  geom_sf(data = secchi.spatial, aes(color = secchi.mean), # put secchi.spatial data on top of the states data in order to see, cause the fill of the states is white
          alpha = 0.5, size = 1) +
...
Maine_combined <- 
  ...
  left_join(., LAGOSlimno) %>%
  left_join(., LAGOSstate) %>%
  filter(state == "ME") %>% #fills out locations where there are no state
  select(lagoslakeid, sampledate, secchi, lake_area_ha, maxdepth, nhd_lat, nhd_long, state)
...
```{r}
secchi_maine.spatial <- st_as_sf(secchi_maine.summary, coords = c("long", "lat"), crs = 4326)# crs is the projection system

Secchi_maine_area_plot <- ggplot() +
  geom_sf(data = states.maine, fill = "white") +
  geom_sf(data = secchi_maine.spatial, aes(color = secchi.mean, size = area), # put secchi.spatial data on top of the states data in order to see, cause the fill of the states is white
          alpha = 0.5) +
  scale_color_viridis_c() +
...
Secchi_maine_depth_plot <- ggplot() +
  geom_sf(data = states.maine, fill = "white") +
  geom_sf(data = secchi_maine.spatial, aes(color = secchi.mean, size = depth), # put secchi.spatial data on top of the states data in order to see, cause the fill of the states is white
          alpha = 0.5) +
  scale_color_viridis_c() +
...
Secchiplot <- ggplot() +
  ...
  geom_sf(data = secchi.decade.spatial, aes(color = secchi.mean), 
          alpha = 0.5, size = 1) +
  facet_wrap(vars(decade), ncol = 5) +# facet wrap
  scale_color_viridis_c() +
  labs(color = "Average Secchi Depth (m)") +
  theme(legend.position = "top")
print(Secchiplot)
```
in 1970, Clean Water Act颁布，测量点增加
...
Secchi.Animation <- ggplot() +
  ...
  theme(legend.position = "top") +
  #gganimate code here:
  labs(title = 'Decade: {closest_state}', color = "Average Secchi Depth (m)") +#Decade: {closest_state}, like decade, 1920 
  transition_states(decade, state_length = 1, transition_length = 0) + # states like the steps
  enter_appear() +
  exit_disappear() 
print(Secchi.Animation)
