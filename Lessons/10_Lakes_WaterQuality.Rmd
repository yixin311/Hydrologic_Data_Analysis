...
> Variable 4 pasture row crops under bufferroom.lulc

> Variable 5: date, season
...
```{r}
# Wrangle data here.
#LAGOS_secchi <- LAGOSdata$epi_nutr$secchi
#LAGOS_chla <-LAGOSdata$epi_nutr$chla
LAGOS_nutri <- LAGOSdata$epi_nutr
LAGOS_state <- LAGOSdata$state
LAGOSlocus <- LAGOSdata$locus
LAGOS_buffer <- LAGOSdata$buffer500m.lulc



LAGOSlocations <- left_join(LAGOSlocus, LAGOS_state, by = "state_zoneid")

LAGOS_new <-
  left_join(LAGOS_nutri, LAGOSlocations,by = "lagoslakeid")%>%
  select(lagoslakeid, sampledate, tn, tp,state,state_name,toc, secchi, chla) %>%
  mutate(sampleyear = year(sampledate), 
         samplemonth = month(sampledate), 
         season = as.factor(quarter(sampledate, fiscal_start = 12)))

LAGOS_pasture <-
  left_join(LAGOS_new, LAGOS_buffer, by = "lagoslakeid")%>%
  select(tn, tp,toc, secchi, chla, buffer500m_nlcd1992_pct_81, buffer500m_nlcd1992_pct_82)%>%
  drop_na()
# no NA data in correlation plot
...
```{r}
corr_par=cor(LAGOS_pasture,method="s")
corrplot(corr_par)
```
