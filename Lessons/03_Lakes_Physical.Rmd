...
# note : qa! get rid of merge conflic

## Opening Discussion
What is a lake? How does this differ from a stream, river, or wetland?

What are the physical properties of lakes?
1. little flow compared to volume
2. vegetation: wetland have merge vegetation
3. light: distinguish shallow wetland from lakes 
...
dim(NTLdata)
NTLdata <- NTLdata %>% 
  drop_na(temperature_C) # drop NA in temperature_C column
dim(NTLdata)   
...
NTLdata <- NTLdata %>%
  filter(lakename %in% c("Paul Lake", "Peter Lake", "Tuesday Lake")) # %in% : includes
...
# Why didn't we use the "summary" function here? cause it is number and it will give max, min, median etc, not data cluster
```
...
TempvsDepth <- 
  ...
  labs(y = expression("Temperature "(degree*C)), x = "Depth (m)") # * degree symbol
print(TempvsDepth)
```
...
<add notes here>
At 9 m depth, very little difference, on the surface layer, great difference
...
<add notes here>
cause we don't know the links between each data, and the line graph connects each point which is not the same as reality.
```{r}
Tempplot.line <- 
  ggplot(Pauldata.surface, aes(x = sampledate, y = temperature_C)) +
...
dimictic: mixes twice a year. as the air cools, the water density becomes smaller between the surface and the bottom. 
...
<add notes here>
1. wind-driven 2. small density gradient
## Closing Discussion
