...
> aggregation: eg: weekly data to monthly data or yearly data. reducing demensions in data sets

> subsampling: cutting data into different periods; by weekly data, random day, when smoothing the data, mistaking the frequency, thus use subsampling rather than aggregation.

> interpolation: interpolate weekly data
...
* **Seasonal Mann-Kendall**: seasonality, non-parametric, no temporal autocorelation, identical distribution. Function: `smk.test` (package: trend); identical distribution: regular and constant gap between samplings. same frequency
...
Mendotadata <- Mendotadata %>%
  ...
  arrange(sampledate) # sorted by date 
...
* **Piecewise constant**: <fill in notes here> the value of nearest neighbor sample
* **Linear**: <fill in notes here> linear interpolation
...
# Generate time series (smk.test needs ts, not data.frame)time series need frequency, start dates and end date
Mendotatimeseries <- ts(linearinterpolation$TP, frequency = 12, 
                        start = c(1988, 7, 5), end = c(2013, 8, 5))
# Run SMK test in trend package
...
> z score means the positive or negative trend, p-value > 0.05, not significant trend; S means the count of the difference between one point to the next; summary gives the each month in different years' trend; ** indicates significance trend; 
> Oct and Nov have significant trend, which are both negative according to z value.
...
ClearCreekDischarge.Monthly <- ClearCreekDischarge %>%
  mutate(Year = year(Date), 
         Month = month(Date)) %>%
  group_by(Year, Month) %>% # why by both year and month? one value for each month for each year
  summarise(Discharge = mean(Discharge))
    
ClearCreek_ts <- ts(ClearCreekDischarge.Monthly[[3]], frequency = 12)# 3 means seasonality
